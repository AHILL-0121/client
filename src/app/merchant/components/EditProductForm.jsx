...
const EditProductForm = ({ selectedProduct, onSubmit, onClose, apiEndpoint }) => {
  ...
  useEffect(() => {
    if (selectedProduct) {
      // Convert comma-separated strings to arrays for size and color
      const sizesArray = selectedProduct.size ? selectedProduct.size.split(',') : [];
      const colorsArray = selectedProduct.color ? selectedProduct.color.split(',') : [];
      ...
      // Setup existing images if available
      if (selectedProduct.product_image) {
        const images = selectedProduct.product_image.split(',').map(img => ({
          preview: img,
          isExisting: true,
          url: img
        }));
  ...
  const handleFileChange = (e) => {
    ...
    // If we want to keep existing images, we'd need to combine
    const existingImages = uploadedFiles.filter(file => file.isExisting);
    setUploadedFiles([...existingImages, ...fileArray]);
  };
    ...
    Object.keys(processedData).forEach((key) => {
      if (key === "product_image") {
        for (let i = 0; i < processedData.product_image.length; i++) {
          updatedProduct.append("product_image", processedData.product_image[i]);
        }
      } else {
        updatedProduct.append(key, processedData[key]);
      }
    });
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {uploadedFiles.map((file, index) => (
                  <div key={index} className="relative">
                    <img
                      src={file.isExisting ? file.url : file.preview}
                      alt={`Product ${index + 1}`}
                        ...
                        onClick={() => {
                          ...
                          if (!file.isExisting) {
                            ...
                            Array.from(formData.product_image).forEach((file, i) => {
                              if (i !== index - uploadedFiles.filter(f => f.isExisting).length) {
