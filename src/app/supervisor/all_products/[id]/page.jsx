...
export default function MerchantProductDetails({ params }) {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${process.env.NEXT_PUBLIC_API_BASE_URL}/sup/Adminsingleproduct/${id}`, {
        headers: {
  ...
  const handleDeleteProduct = async (productId) => {
    ...
    if (!token) {
      router.push("/supervisor/login"); // Redirect if no token found
      return;
    }

    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/sup/productadmin/${productId}`,
        {
          method: "DELETE",
          headers: {
      ...
      if (response.ok) {
        ...
        router.push("/supervisor/all_products"); // Redirect after deletion
      } else {
