...
export default function OverviewPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // If there's no admin token or if there's a supervisor token, redirect to login
        if (!token || supervisorToken) {
            localStorage.removeItem("supervisorToken"); // Clean up any supervisor token
        ...
        const salesResponse = await axios.get(
          `${apiBaseUrl}/admin/salestrend?time_frame=${timeFrame}`,
          { headers }
        );
        ...
        const ordersResponse = await axios.get(
          `${apiBaseUrl}/admin/ordertrend?time_frame=${timeFrame}`,
          { headers }
        );
  ...
  const salesChartData = {
    labels: salesLabels.length > 0 ? salesLabels : Array.from({ length: salesTrend.length }, (_, i) => `Day ${i + 1}`),
    datasets: [
      {
        label: "Sales",
        data: salesTrend,
  ...
  const orderChartData = {
    labels: orderLabels.length > 0 ? orderLabels : Array.from({ length: orderTrend.length }, (_, i) => `Day ${i + 1}`),
    datasets: [
      {
        label: "Orders",
        data: orderTrend,
