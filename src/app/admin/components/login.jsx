  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setPopupMessage("Login successful!");
        setShowPopup(true);
      localStorage.setItem("adminToken", response.data.token); // Store the JWT token in localStorage
      localStorage.removeItem("supervisorToken");
      router.push("/admin/overview"); // Redirect to the dashboard or another page
    } catch (error) {
      if (error.response) {
        // Check if the backend returns a specific error message for pending requests
        if (error.response.data.error === "pending") {
          setPopupMessage("Admin process in progress, please wait.");
          setShowPopup(true);
        } else {
