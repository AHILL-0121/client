...
const CustomerManagement = () => {
  ...
  const fetchAllCustomers = async () => {
    try {
      ...
      const response = await axios.get(`${apiBaseUrl}/getallusers`, {
  ...
  const handleSearchByUsername = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${apiBaseUrl}/searchuser?username=${searchUsername.trim()}`,
        {
          headers: {
  ...
  const handleSearchByUserID = async () => {
    ...
    try {
      ...
      const token = localStorage.getItem("adminToken");
      const response = await axios.get(
        `${apiBaseUrl}/searchuser?user_id=${searchUserID.trim()}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleDeleteCustomer = async () => {
    ...
    try {
      setIsDeleting(true);
      const token = localStorage.getItem("adminToken");
      await axios.delete(`${apiBaseUrl}/deleteuser/${selectedCustomerId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleViewCustomer = (user_id) => {
    router.push(`/admin/customer_management/${user_id}`);
  };
